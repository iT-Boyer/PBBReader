...
-(void)searchWhere:(NSString *)where orderBy:(NSString *)orderBy callback:(void (^)(NSArray *))block
{
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         NSMutableString* query = [NSMutableString stringWithFormat:@"select rowid,* from %@ ",[self.class getTableName]];
...
-(void)searchTableNum:(NSString *)logname callback:(void(^)(int))block
{
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         NSMutableString* query = [NSMutableString stringWithFormat:@"select count(*) from %@ where logName = '%@';",[self.class getTableName],logname];
    ...
    while ([set next]) {
        ...
        for (int i=0; i<self.columeNames.count; i++) {
            ...
            else if([columeType isEqualToString:@"NSString"])
            {
                ...
                //过滤特殊字符串
                if([columeName isEqualToString:@"description"]||
                   [columeName isEqualToString:@"debugDescription"]){
                    continue;
                }
            ...
            else if([columeType isEqualToString:@"NSDate"])
            {
                ...
                NSDate *dat = [LKDAOBase dateWithString:datestr];
                [bindingModel setValue:dat forKey:columeName];
            }
    ...
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         ...
         if(model.rowid > 0)
         {
             updateSQL = [NSString stringWithFormat:@"update %@ set %@ where rowid=%ld",[self.class getTableName],updateKey,(long)model.rowid];
         }
         else
         {
         ...
         if(execute == NO)
         {
             NSLog(@"database update fail %@   ----->rowid: %ld",NSStringFromClass(model.class),(long)model.rowid);
         }
     }];
    ...
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         ...
         if(model.rowid > 0)
         {
             delete = [NSString stringWithFormat:@"DELETE FROM %@ where rowid=%ld",[self.class getTableName],(long)model.rowid];
