...
void makekey(register char *aptr, register unsigned char *kptr)
	//aptr;     /* NULL-terminated */
	//kptr;   /* unsigned char[8] */
{
    unsigned char *store;
    int first, i;
...
**********************************************************************/
int maccrypt(const char *input,const int length,char *mac,char *key)
{
	char outbuf[9];
	char inbuf[9];
	char *plain,*plain2;
	...
	plain=(char *)malloc(length+8);
	memset(plain,0x00,length+8);
	...
	for(i=0;i<times;i++)
	{
		...
		if(0==i)
		{
			memcpy(inbuf,plain2,8);
			des((unsigned char *)inbuf,(unsigned char *)outbuf);
		}
		else
		{
			for(j=0;j<8;j++)
